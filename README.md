...
2. Run the schema migrations, in order, against that database:
   ...
   psql "$DATABASE_URL" -f migrations/046_team_member_partners_access.sql
   psql "$DATABASE_URL" -f migrations/047_notes_editing.sql
   ```
...
migrations/046_team_member_partners_access.sql  adds team_members.has_partners_access (boolean, default false) — lets the owner grant individual team members access to the Partners section, which is owner-only by default. A single on/off flag rather than a per-partner picklist like client_access (Partners has no per-record scoping concept) — granted via the same "manage login and access" panel in Settings → Team Members that already handles client access (src/components/settings/TeamMembersPanel.tsx). A granted team member gets the same read/write access as the owner to every partner (notes, commissions, etc.) except creating a brand-new partner, which stays owner-only. See requirePartnerAccess in src/lib/currentUser.ts and the /partners page/[id] routes, which now check this flag instead of being blanket-blocked in proxy.ts (proxy.ts no longer lists /partners as an owner-only prefix)
migrations/047_notes_editing.sql  adds the partner_notes table (mirrors client_notes/lead_notes exactly, author attribution included) and gives Partners its own "Notes & Activity" tab, matching Clients/Leads — previously Partners only had a single free-text `notes` field on the record itself. Any existing text in that field is migrated over as the partner's first log entry, then the column is dropped, so nothing is lost. Also (no schema change) adds the ability to edit a note after adding it, for all three record types — see updateClientNote/updateLeadNote/updatePartnerNote in the respective actions.ts files, the edit affordance in src/components/NotesLog.tsx, and the defaultValue support added to src/components/MentionTextarea.tsx so an edited note's existing @mention chips re-render correctly rather than needing to be retyped
