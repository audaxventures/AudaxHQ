...
function mapDocument(row: Record<string, unknown>): ClientDocument {
  return {
    id: row.id as string,
...
export async function createDocument(input: DocumentInput): Promise<ClientDocument> {
  const rows = await sql`
    insert into documents (client_id, file_name, file_path, file_type, file_size, label, uploaded_by)
    values (
      ${input.clientId}, ${input.fileName}, ${input.filePath}, ${input.fileType},
...
// Namespaced by client so a bucket listing groups by client, and given a
// random prefix so two uploads of the same filename never collide.
export function newDocumentStoragePath(clientId: string, fileName: string): string {
  return `${clientId}/${randomUUID()}-${fileName}`;
}
